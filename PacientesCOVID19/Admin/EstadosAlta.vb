Public Class EstadosAlta
    '0 = consulta, 1 = agregar, 2 = editar
    Public IntTipoForm As Integer

    Private Sub EstadosAlta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Button_Agregar_Click(sender As Object, e As EventArgs) Handles Button_Agregar.Click
        If Button_Agregar.Text = "Agregar" Then
            ...
            Me.EstadosBindingSource.AddNew()
            IntTipoForm = 1
            TextBox_Estado.Select()
        Else
            fx_activarbotones(False)
            Me.EstadosBindingSource.CancelEdit()
            IntTipoForm = 0
        End If
    End Sub
        ...
        Else
            If IntTipoForm = 1 Then
                ...
                If fx_validarcampos() Then
                    Try
                        Me.EstadosTableAdapter.InsertQuery(TextBox_Estado.Text)
                        fx_activarbotones(False)
                        IntTipoForm = 0
                        fx_buscar()
            ...
            ElseIf IntTipoForm = 2 Then
                ...
                If fx_validarcampos() Then
                    Try
                        Me.EstadosTableAdapter.UpdateQuery(TextBox_Estado.Text, idestado_tmp)
                        fx_activarbotones(False)
                        IntTipoForm = 0
                        fx_buscar()
                    Catch ex As Exception
    ...
    Private Sub Button_Eliminar_Click(sender As Object, e As EventArgs) Handles Button_Eliminar.Click
        If MsgBox("Esta seguro que desea eliminar al registro: " + TextBox_Estado.Text + " ?", MsgBoxStyle.YesNoCancel, "Eliminando registro") = DialogResult.Yes Then
            Me.EstadosTableAdapter.DeleteQuery(TextBox_ID.Text)
            fx_buscar()
        End If
    End Sub
